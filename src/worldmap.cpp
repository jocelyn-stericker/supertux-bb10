	    ...
	    case SDL_JOYHATMOTION:
	      ...
	      break;
            case SDL_MOUSEBUTTONDOWN:
              {
                  Point offset;
		      offset.x = -tux->get_pos().x + screen->w/2;
		      offset.y = -tux->get_pos().y + screen->h/2;
		      ...
		      int x = event.motion.x - offset.x;
		      int y = event.motion.y - offset.y;

                  int xdx = (x - tux->get_pos().x);
                  int ydy = (y - tux->get_pos().y);
                  if (xdx*xdx + ydy*ydy < 650) {
                      enter_level = true;
		  } else {
			  ...
			  if (tile->north && ydy < 0)
				  dir = D_NORTH;
			  else if (tile->south && ydy > 0)
				  dir = D_SOUTH;
			  else if (tile->east && xdx > 0)
				  dir = D_EAST;
			  else if (tile->west && xdx < 0)
				  dir = D_WEST;
			input_direction = dir;
                  }
              }
...
WorldMap::draw_status()
{
  ...
  white_small_text->draw("SCORE", 0, 0);
  white_small_text->draw(str, 96, 0);

  sprintf(str, "%d", player_status.distros);
  white_small_text->draw_align("COINS", 320-64, 0,  A_LEFT, A_TOP);
  white_small_text->draw_align(str, 320+64, 0, A_RIGHT, A_TOP);

  white_small_text->draw("LIVES", 480, 0);
  if (player_status.lives >= 5)
    {
      sprintf(str, "%dx", player_status.lives);
      white_small_text->draw_align(str, 617, 0, A_RIGHT, A_TOP);
