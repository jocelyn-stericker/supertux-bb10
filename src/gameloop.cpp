...
GameSession::levelintro(void)
{
  ...
  white_small_text->drawf(str, 0, 224, A_HMIDDLE, A_TOP, 1);
      ...
      while (SDL_PollEvent(&event))
        {
	      ...
	      st_pause_ticks_stop();
            ...
            {
              static int LEFT_MB = -1, RIGHT_MB = -1, UP_MB = -1, RUN_MB = -1, DOWN_MB = -1;
              Player& tux = *world->get_tux();
		...
		case SDL_MOUSEBUTTONDOWN:
		  {
			  ...
			  int y = event.motion.y;
                          if (y > 280 && x <= 210) {
                                  if ( (x- 60)*(x-60) + (y - 320-115)*(y-320-115) < (x-160)*(x-160) + (y-320-115)*(y-320-115)) {
					tux.key_event((SDLKey)keymap.left, DOWN);
                                        LEFT_MB = event.button.which; 
				  } else {
					tux.key_event((SDLKey)keymap.right, DOWN);
                                        RIGHT_MB = event.button.which; 
				}
			  } else if (y > 280 && x >= 800-220) {
                                  if (( (x- 800+220-40)*(x-800+220-40) + (y - 320-115)*(y-320-115) < (x-800+220-120)*(x-800+220-120) + (y-320-115)*(y-320-115)) && ( (x- 800+220-40)*(x-800+220-40) + (y - 320-115)*(y-320-115) < (x-800+220-180)*(x-800+220-180) + (y-320-115)*(y-320-115))) {
					tux.key_event((SDLKey)keymap.jump, DOWN);
                                        UP_MB = event.button.which; 
                                  } else if ((x-800+220-120)*(x-800+220-120) + (y-320-115)*(y-320-115) < (x-800+220-180)*(x-800+220-180) + (y-320-115)*(y-320-115)) {
					tux.key_event((SDLKey)keymap.duck, DOWN);
                                        DOWN_MB = event.button.which; 
				} else {
					tux.key_event((SDLKey)keymap.fire, DOWN);
                                        RUN_MB = event.button.which; 
				}
			  }
		  }
		  break;
		case SDL_MOUSEBUTTONUP:
		  {
			if (LEFT_MB == event.button.which) {
				LEFT_MB = -1;
				tux.key_event((SDLKey)keymap.left, UP);
			}
			if (RIGHT_MB == event.button.which) {
				RIGHT_MB = -1;
				tux.key_event((SDLKey)keymap.right, UP);
			}
			if (UP_MB == event.button.which) {
				UP_MB = -1;
				tux.key_event((SDLKey)keymap.jump, UP);
			}
			if (DOWN_MB == event.button.which) {
				DOWN_MB = -1;
				tux.key_event((SDLKey)keymap.duck, UP);
			}
			if (RUN_MB == event.button.which) {
				RUN_MB = -1;
				tux.key_event((SDLKey)keymap.fire, UP);
			}
		  }
  ...
  if(game_pause)
    {
      ...
      white_small_text->drawf("PAUSE - Press 'P' To Play", 0, 230, A_HMIDDLE, A_TOP, 1);
    }
...
GameSession::drawstatus()
{
  ...
  white_small_text->draw("SCORE", 0, 0, 1);
  white_small_text->draw(str, 96, 0, 1);

  if(st_gl_mode == ST_GL_TEST)
    {
      white_small_text->draw("Press ESC To Return",0,20,1);
    }

  if(!time_left.check()) {
    white_small_text->draw("TIME'S UP", 224, 0, 1);
  } else if (time_left.get_left() > TIME_WARNING || (global_frame_counter % 10) < 5) {
    sprintf(str, "%d", time_left.get_left() / 1000 );
    white_small_text->draw("TIME", 224, 0, 1);
    white_small_text->draw(str, 304, 0, 1);
  }

  sprintf(str, "%d", player_status.distros);
  white_small_text->draw("COINS", screen->h, 0, 1);
  white_small_text->draw(str, 608, 0, 1);

  white_small_text->draw("LIVES", 480, 20);
  if (player_status.lives >= 5)
    {
      sprintf(str, "%dx", player_status.lives);
      white_small_text->draw_align(str, 617, 20, A_RIGHT, A_TOP);
  ...
  if(show_fps)
    {
      ...
      white_small_text->draw("FPS", screen->h, 40, 1);
      white_small_text->draw(str, screen->h + 60, 40, 1);
    }

  keypad->draw(10, 320);
  keypad2->draw(800-220, 320);
}
...
GameSession::drawendscreen()
{
  ...
  white_small_text->drawf("GAMEOVER", 0, 200, A_HMIDDLE, A_TOP, 1);

  sprintf(str, "SCORE: %d", player_status.score);
  white_small_text->drawf(str, 0, 224, A_HMIDDLE, A_TOP, 1);

  sprintf(str, "COINS: %d", player_status.distros);
  white_small_text->drawf(str, 0, 256, A_HMIDDLE, A_TOP, 1);
...
GameSession::drawresultscreen(void)
{
  ...
  white_small_text->drawf("Result:", 0, 200, A_HMIDDLE, A_TOP, 1);
